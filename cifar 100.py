...
# dataset settings
parser.add_argument('--noise_mode', default='asym', type=str, help='artifical noise mode (default: symmetric)')  # asym  sym
parser.add_argument('--noise_ratio', default=0.3, type=float, help='artifical noise ratio (default: 0.5)')
...
parser.add_argument('--k', default=500, type=int, help='neighbors for knn sample selection (default: 200)')

# train settings
parser.add_argument('--epochs', default=50, type=int, metavar='N', help='number of total epochs to run (default: 300)')
...
def evaluate(dataloader, encoder, classifier, args, noisy_label, clean_label, i, stat_logs):
    ...
    with torch.no_grad():
        ...
        def combined_entropy_margin(prediction_cls):
            ...
            combined_score = entropy / (margin + 1e-5)  # 结合熵和边距
