...
def knn_weighted(feature, feature_bank, feature_labels, classes, knn_k):
    # compute cos similarity between each feature vector and feature bank ---> [B, N]
    sim_matrix = torch.mm(feature, feature_bank)
    sim_weight, sim_indices = sim_matrix.topk(k=knn_k, dim=-1)
    sim_labels = torch.gather(feature_labels.expand(feature.size(0), -1), dim=-1, index=sim_indices)

    min_value = sim_weight.min(dim = -1, keepdim = True)[0]
    max_value = sim_weight.max(dim = -1, keepdim = True)[0]
    sim_weight = (sim_weight - min_value) / (max_value - min_value + 1e-10)
...
def weighted_knn(cur_feature, feature, label, num_classes, knn_k=100, chunks=10, norm='global'):
    ...
    with torch.no_grad():
        for i in range(chunks):
            ...
            part_score, part_pred = knn_weighted(part_feature, feature.T, label, num_classes, knn_k)
